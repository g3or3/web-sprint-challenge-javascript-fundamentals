...
/* 
    Inside of myFunction there is one declared variable `internal` which in JS can be accessed by any other functions WITHIN the scope of myFunction. 
    Functions nested within other functions in JS form closures which allow the inner functions to have access to the outer function's variables. 
*/
...
function animalNames(zoo){
  let displayNames = [];
  zoo.forEach(animal => displayNames.push(`name: ${animal.animal_name}, scientific: ${animal.scientific_name}`));
  return displayNames;
}
...
function USApop(zoo){
  return zoo.reduce((accum, animal) => {
